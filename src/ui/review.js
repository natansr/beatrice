...
export async function renderReview(root, projectId, pageId) {
  ...
  root.innerHTML = shell(`<section class="review-head"><a href="#/project/${project.id}">← ${escapeHtml(project.title)}</a><div>${previous ? `<a class="button" href="#/project/${project.id}/page/${previous.id}">${t('previous')}</a>` : ''}<label>${t('page')} <input id="page-number" type="number" value="${page.pageNumber}" min="1"></label>${next ? `<a class="button" href="#/project/${project.id}/page/${next.id}">${t('next')}</a>` : ''}</div><span id="save-state">${t('saved')}</span></section>
    ...
    <article class="text-panel"><div class="toolbar"><button id="process" title="${t('processHelp')}">${t('processImage')}</button><button id="ocr" title="${t('ocrMeaning')}">${t('runOcr')}</button><button id="needs" class="warning-button">${t('markReview')}</button><button id="reviewed" class="primary">${t('markReviewed')}</button></div><details class="inline-help"><summary>${t('beginnerTitle')}</summary><p>${t('reviewHelp')}</p><p>${t('uncertaintyHelp')}</p><p>${t('processHelp')}</p></details><div id="ocr-progress" class="ocr-progress" hidden><div></div><span></span></div><label class="sr-only" for="reviewed-text">${t('transcription')}</label><textarea id="reviewed-text" spellcheck="false" placeholder="${t('transcriptionPlaceholder')}">${escapeHtml(page.reviewedText)}</textarea><footer><em id="status" class="status ${page.status}">${statusLabel(page.status)}</em><span>${page.ocrConfidence != null ? t('ocrConfidence', { value: page.ocrConfidence.toFixed(1) }) : ''}</span></footer></article></section>`)
